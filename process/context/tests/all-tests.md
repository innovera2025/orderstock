...
description: Testing entrypoint for orderstock — Vitest 3.2.6 (82 tests/15 files) and Playwright E2E (21 tests) both real and wired, sandbox SQL Server constraint
...
Last updated: 2026-07-08 (pguard-redesign Phase 03 closeout — New Screens ✅ VERIFIED)
...
**Real, wired, and passing as of pguard-redesign Phase 03 (New Screens, ✅ VERIFIED 08-07-26).** phase1-order-system remains program-complete (its own baseline: 70 tests / 12 files, 19 e2e). The pguard-redesign program has since added to the SAME suites: Vitest is now green with **82 tests across 15 files** (order-payload unit added Phase 02, summary unit added Phase 03), and Playwright is now **21 e2e tests across 5 spec files** (the prior 4 unchanged + new `e2e/summary-history.spec.ts`, 2 tests, Phase 03). A Docker SQL Server 2022 sandbox provides the DB-dependent gates, plus a second same-container database (`orderstock2`) used only by the Phase 06 Hybrid round-trip gate. This file's Commands table below is the current baseline; any future order-system OR pguard-redesign work should extend it the same way.
...
| root | Playwright | `pnpm exec playwright test` | Yes — needs sandbox up + seeded admin/data (`SEED_ADMIN_PASSWORD` in `.env`); `webServer` auto-starts `pnpm start`; 21/21 as of pguard-redesign Phase 03 |
...
**pguard-redesign Phase 02 residual (non-blocking, e2e infra) — pattern now demonstrated by Phase 03:**
...
- **`e2e/orders.spec.ts` is first-run-clean, not idempotent.** `createOrderSheet` dedups by
  ...
  no order-dependence) — the gap only bites recovery-from-failure. **`e2e/summary-history.spec.ts`
  (Phase 03) now demonstrates the fix** — a `beforeEach`+`afterAll` clean-state helper (delete
  E2E-located sheets, restore renamed shops). `e2e/orders.spec.ts` itself has NOT been retrofitted
  with this helper yet (still an open item); Phase 04's new e2e specs should adopt the same
  pattern, and hoisting it into a shared e2e util (rather than duplicating per-spec) is a
  follow-up stub from Phase 03 EVL.
