...
description: Testing entrypoint for orderstock — Vitest 3.2.6 (70 tests/14 files) and Playwright E2E (39 tests excl. [setup], incl. mobile + tablet projects) both real and wired, sandbox SQL Server constraint
...
Last updated: 2026-07-13 (`matrix-print-darkmode-fixes` plan ✅ VERIFIED at code level — added new print e2e gate G9, no unit test change; prior: `remove-settings-db` plan VERIFIED — deleted 29 unit tests / 3 e2e tests with the removed `/settings/db` page; `responsive-drawer-sidebar` plan CODE DONE — added 8 new e2e gates across a new `tablet` project + a new `chromium`-tier spec)
...
| root | Playwright | `pnpm exec playwright test` | Yes — needs sandbox up + seeded admin/data (`SEED_ADMIN_PASSWORD` in `.env`); `webServer` auto-starts `pnpm start`; 39 tests excl. `[setup]` as of `matrix-print-darkmode-fixes_13-07-26` (`chromium` desktop + `mobile` + `tablet` projects) |
