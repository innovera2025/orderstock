...
description: Testing entrypoint for orderstock — Vitest 3.2.6 (70 tests/14 files) and Playwright E2E (38 tests excl. [setup], incl. mobile + tablet projects) both real and wired, sandbox SQL Server constraint
keywords: tests, testing, vitest, playwright, e2e, unit, integration, verification, coverage, sandbox, sql server, health check, build, lint, storage state, fixtures, totals, be-date, order-save, mobile, mobile viewport, mobile project, tablet, tablet project, sidebar drawer, clean-state
...
Last updated: 2026-07-11 (`remove-settings-db` plan VERIFIED — deleted 29 unit tests / 3 e2e tests with the removed `/settings/db` page; `responsive-drawer-sidebar` plan CODE DONE — added 8 new e2e gates across a new `tablet` project + a new `chromium`-tier spec)
...
| root | Playwright | `pnpm exec playwright test` | Yes — needs sandbox up + seeded admin/data (`SEED_ADMIN_PASSWORD` in `.env`); `webServer` auto-starts `pnpm start`; 38 tests excl. `[setup]` as of `responsive-drawer-sidebar_11-07-26` (`chromium` desktop + `mobile` + `tablet` projects) |
