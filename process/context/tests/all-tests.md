...
description: Testing entrypoint for orderstock — Vitest 3.2.6 (75 tests/14 files) and Playwright E2E (19 tests) both real and wired, sandbox SQL Server constraint
...
Last updated: 2026-07-07 (pguard-redesign Phase 02 closeout — Core Desktop ✅ VERIFIED)
...
**pguard-redesign Phase 02 residual (non-blocking, e2e infra):**
...
- **`e2e/orders.spec.ts` is first-run-clean, not idempotent.** `createOrderSheet` dedups by
  ...
  no order-dependence) — the gap only bites recovery-from-failure. Recommended fix: a
  `beforeEach` that deletes E2E-located sheets and restores the renamed shop's name before each
  run. Phase 03/04 (both adding new e2e specs on the same matrix-derived pattern) should adopt
  this `beforeEach` clean-state helper.
