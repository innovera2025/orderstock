...
Last updated: 2026-07-07 (Phase 06 / program closeout — phase1-order-system COMPLETE)
...
**Real, wired, and passing as of Phase 06 (DB Settings & Delivery) — phase1-order-system is program-complete.** Vitest 3.2.6 is installed and green with 70 tests across 12 files. Playwright `@playwright/test@1.61.1` is installed and green with 19 tests across `e2e/auth.spec.ts` + `e2e/orders.spec.ts` + `e2e/print.spec.ts` + `e2e/settings.spec.ts`. A Docker SQL Server 2022 sandbox provides the DB-dependent gates, plus a second same-container database (`orderstock2`) used only by the Phase 06 Hybrid round-trip gate. This file's Commands table below is the current baseline for this program; any future order-system work should extend it the same way.

## Testing Approach

Stack: Next.js 16.2.10 (TypeScript) + Prisma 7 + `@prisma/adapter-mssql` + SQL Server (see `all-context.md`):

- **Unit/integration:** Vitest 3.2.6 — `src/lib/__tests__/*.test.ts` (+ one settings-specific spec). 12 files / 70 tests as of Phase 06 (program complete): `smoke.test.ts` (baseline), `variant-validation.test.ts` (5 — printOrder uniqueness over the active non-null set), `correction-cascade.test.ts` (2 — propagate-while-unconfirmed / lock-after-confirm branches), `password.test.ts` (4 — bcryptjs hash/verify round-trip, wrong password, 72-byte limit), `login-attempts.test.ts` (5 — lockout BLOCK/RESET/EXPIRE), `auth-guard-coverage.test.ts` (10, Phase 06 +3 — grep-style assertion that every shop/product/admin/order/settings action AND every settings/print page call `requireAuth`; MODULES paths re-point to `src/app/(main)/…`, with a dedicated `ADMIN_MODULES` assertion for the settings actions module), `secret-leak.test.ts` (2 — no committed plaintext secret in tracked files), `totals.test.ts` (6, Phase 04 — asserts all 20 column totals + grand 446 against `test-fixtures/sheet-13-03-69.json`, NoteLine qty excluded, weight computation shape), `be-date.test.ts` (5, Phase 04 — CE↔BE round-trip via Intl `en-US-u-ca-buddhist`), `order-save.test.ts` (3, Phase 04 — `mergeSnapshots()` proves naive re-derive-from-live-names FAILS while carry-forward passes), `connection-string.test.ts` (14, Phase 06 — fields→`sqlserver://` build, named-instance, brace-escaping, validation, mask, non-load-bearing parse), `env-write.test.ts` (7, Phase 06 — injection-safe serialization against hostile inputs incl. `\n`/`\nKEY=value` clobber/quotes/backslashes/Thai password, `.env.bak` backup-before-write ordering, gitignore assertion), `settings-secret-hygiene.test.ts` (5, Phase 06 — password masking + test-connection error sanitization).
...
| root | Playwright | `pnpm exec playwright test` | Yes — needs sandbox up + seeded admin/data (`SEED_ADMIN_PASSWORD` in `.env`); `webServer` auto-starts `pnpm start`; 19/19 as of Phase 06 |
