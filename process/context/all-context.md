...
Last updated: 2026-07-15 (`location-management` plan ✅ VERIFIED AT CODE LEVEL — pending commit —
...
| `tests/` | `process/context/tests/all-tests.md` | Testing entrypoint for orderstock — Vitest 3.2.6 (92 tests/16 files) and Playwright E2E (46 tests excl. [setup], incl. mobile + tablet projects) both real and wired, sandbox SQL Server constraint |
...
|---|---|---|
| `order-system` | `process/features/order-system/` | **COMPLETE (07-07-26); +4 follow-up plans VERIFIED/verified-at-code-level (latest 15-07-26)** — phase program `phase1-order-system_06-07-26` (umbrella + 6 phase plans), all 6 phases ✅ VERIFIED, archived to `completed/`. Follow-up plans `ordersheet-soft-delete_11-07-26` (ADMIN-only OrderSheet soft-delete, additive `active` column), `remove-settings-db_11-07-26` (removed the `/settings/db` runtime DB-connection page), `shop-location-roster_13-07-26` (`Shop.location` + per-location roster via `src/lib/roster.ts`, db_TCL delivery script `db/alter-shop-add-location.sql` pending), and `location-management_14-07-26` (managed `/locations` list via a single JSON `AppSetting` row, zero schema change, feeding both the shop-form and order-sheet location pickers) all archived to `completed/`; `location-management` is ✅ VERIFIED AT CODE LEVEL (all Fully-Automated/Hybrid gates green, user confirmation of live `/locations` UX pending-manual). A related dark-mode/print-footer fix (`matrix-print-darkmode-fixes_13-07-26`) lives in `process/general-plans/completed/`, not this feature folder. 7 backlog items remain, pending customer answers or optional hardening — see `process/features/order-system/backlog/` |
  ...
  src/
    ...
    app/
      ...
      (main)/                   -- Phase 05: route GROUP (adds no URL segment) holding every authenticated app route; layout.tsx composes sidebar (nav.tsx) + topbar.tsx + scrollable <main> (pguard-redesign Phase 01)
        ...
        shops/**                -- shops master-data CRUD (Phase 02; requireAuth-guarded since Phase 03); shop-location-roster_13-07-26 added an optional "สถานที่" (`location`) input to shop-form.tsx + a `.max(200)` zod field in actions.ts
        ...
        locations/**            -- location-management_14-07-26 (✅ VERIFIED AT CODE LEVEL, NEW): managed สถานที่ list page — page.tsx (server, requireAuth no role, force-dynamic), actions.ts (createLocationAction/renameLocationAction/deleteLocationAction), locations-manager.tsx (client, per-row LocationRow subcomponent each with its own useActionState — mirrors delete-sheet-button.tsx's confirm-modal pattern); rename cascades atomically to Shop.location via prisma.$transaction; delete blocked while any shop (active OR soft-deleted) still references the location
          ...
          new-sheet-form.tsx     -- native date input (CE) + read-only BE label + "วันนี้" shortcut; shop-location-roster_13-07-26: location field is now a `<select>` of distinct, `active:true`-filtered shop locations (was free text)
...
- Generated: 2026-07-15 (`location-management` plan ✅ VERIFIED AT CODE LEVEL, archived — UPDATE-PROCESS)
- HEAD: e0a4a88 (pending commit for location-management changes — see git status)
- Mode: delta update (managed /locations list + AppSetting-backed location picker; test-count refresh 75→92 unit, 42→46 e2e excl. setup)
