...
Last updated: 2026-07-17 (`shop-location-filter` plan ✅ VERIFIED AT CODE LEVEL and archived —
...
| `tests/` | `process/context/tests/all-tests.md` | Testing entrypoint for orderstock — Vitest 3.2.6 (92 tests/16 files) and Playwright E2E (48 tests excl. [setup], incl. mobile + tablet projects) both real and wired, sandbox SQL Server constraint |
...
orderstock/
  ...
  src/
    ...
    lib/
      ...
      __tests__/                -- smoke, variant-validation, correction-cascade, password, login-attempts, auth-guard-coverage (now covers (main)/ + print pages + settings), secret-leak, totals, be-date, order-save, roster, locations (NEW, location-management_14-07-26 — pure-transform unit tests for locations-core.ts) (92 tests total/16 files, after remove-settings-db plan removed connection-string.test.ts + env-write.test.ts + settings-secret-hygiene.test.ts, then shop-location-roster_13-07-26 added roster.test.ts +5, then location-management_14-07-26 added locations.test.ts +17)
  test-fixtures/
...
- **Testing:** Vitest 3.2.6 (`pnpm test` → `vitest run`) — 92 tests across 16 files (grew to 99/17 through the `ordersheet-soft-delete_11-07-26` plan; the `remove-settings-db_11-07-26` plan then DELETED `connection-string.test.ts`/`env-write.test.ts`/`settings-secret-hygiene.test.ts` and pruned 3 `auth-guard-coverage.test.ts` assertions, netting 99/17 → 70/14; `shop-location-roster_13-07-26` then ADDED `roster.test.ts` (+5), netting 70/14 → 75/15; `location-management_14-07-26` then ADDED `locations.test.ts` (+17), netting 75/15 → 92/16; `shop-location-filter_17-07-26` added no new unit tests — netting still **92/16**). Playwright `@playwright/test@1.61.1` — 48 E2E tests (excl. `[setup]`, 49 incl.) across `e2e/auth.spec.ts` + `e2e/orders.spec.ts` + `e2e/print.spec.ts` + `e2e/summary-history.spec.ts` + `e2e/mobile.spec.ts` (390×844 `mobile` project) + `e2e/shops.spec.ts` (NEW, `shop-location-filter_17-07-26`) + `e2e/sidebar-drawer.spec.ts` (820×1180 `tablet` project) + `e2e/sidebar-desktop-collapse.spec.ts` (`chromium` project) + `e2e/auth.setup.ts` fixtures — `remove-settings-db_11-07-26` DELETED `e2e/settings.spec.ts` (34→30), `responsive-drawer-sidebar_11-07-26` added the new `tablet` project spec (5 gates) + the new desktop-collapse spec (3 gates) (30→38), `matrix-print-darkmode-fixes_13-07-26` added print gate G9 (38→39), `shop-location-roster_13-07-26` added G4/G5 (`orders.spec.ts`) + G6 (`print.spec.ts`) (39→42), `location-management_14-07-26` added 4 new location-management gates to `e2e/orders.spec.ts` (42→46), then `shop-location-filter_17-07-26` added 1 new gate to `e2e/orders.spec.ts` + 1 new gate in the new `e2e/shops.spec.ts` (46→**48**). See `tests/all-tests.md`. **pguard-redesign PROGRAM COMPLETE (08-07-26); order-system `ordersheet-soft-delete`, `remove-settings-db`, `shop-location-roster`, `location-management`, and `shop-location-filter` follow-up plans VERIFIED/verified-at-code-level (latest 17-07-26); `responsive-drawer-sidebar` plan CODE DONE (11-07-26, Agent-Probe rows pending-manual).**
...
- Generated: 2026-07-17 (`shop-location-filter` plan ✅ VERIFIED AT CODE LEVEL, archived — UPDATE-PROCESS)
- HEAD: e0a4a88 (pending commit for shop-location-filter changes — see git status; a separate vc-git-manager pass handles the commit)
- Mode: delta update (per-location list filtering on /shops + /orders via ?location=; test-count refresh 46→48 e2e excl. setup, unit unchanged at 92/16)
