...
Last updated: 2026-09-22 (`erp-dashboards` **PROGRAM COMPLETE** — Phase 5 "Hardening, Export &
Rollout" ✅ VERIFIED at agent level, program archived to
`process/features/erp-dashboards/completed/erp-dashboards_18-09-26/`. Phase 5: CSV export shipped
on all six dashboard-table call sites (UTF-8 BOM, CRLF, Thai headers, BE dates, 5,000-row cap with
visible truncation notice, ASCII filename, STAFF money columns omitted entirely — proven at 3
independent levels incl. a role-diffed rendered-DOM check); cross-dashboard AC9 money audit (98
tests, all 3 dashboards + exports); the program's first real ERP force-down mechanism
(`src/lib/erp/force-down.ts` + gated `/api/test/erp-force-down`) proving AC15/AC16 degraded-mode
behavior across all three dashboards at once; AC17 re-confirmed plus a permanent automated sweep of
the real `assertReadOnlySql` guard over all 13 shipped `db/erp-queries/**/*.sql` files;
`production-seed.sql`/`purchase-seed.sql` order-dependence bug fixed with a red-then-green
idempotency regression gate; a manual live-`db_TCL` reconcile script written (`live-reconcile-script.ts`
+ `scripts/erp-reconcile.ts`), refusal gates exercised, fixture-verified against every known truth,
**never run against `db_TCL`** (USER-RUN by charter); rollout-readiness REF + deployment guide §12
written. Full regression: Vitest 523 passed/0 failed with ERP fixture wired (490/33 self-skipped
without), Playwright 145 passed/7 skipped vs the 119/7 baseline = exactly +26 new tests, **zero
regressions**; lint/build/typecheck clean. EVL: 1 orchestrator-run cycle (not a subagent — see
tooling lesson below), `all_pass: true` on first confirmation, zero fix cycles needed. Known gaps:
2 permanent USER-RUN items (DBA-provisioned scoped read-only login; the manual live reconcile
itself) gate production go-live only, not agent-level program completion. **Correction to Phase 3's
prior "9 Hybrid/Agent-Probe gates env-blocked" known-gap:** independently CLOSED this session — the
orchestrator's own full-suite run (not a `vc-tester` subagent) reproduced all 9 gates green.
**Durable tooling lesson:** every `vc-tester`/EVL SUBAGENT sandbox in this harness is blocked from
materializing `ERP_DATABASE_URL`/`ERP_ALLOW_WRITE_CAPABLE_LOGIN` (recurred identically across
Phases 2, 3, and 5) — ERP-env-dependent gates must be confirmed by the orchestrator or the user
running the suite directly with the env inline (see `tests/all-tests.md`), not by a `vc-tester`
subagent. Zero schema change across the whole program; no ERP table ever entered
`prisma/schema.prisma`; `db_TCL` never contacted by any automated run. Nothing from this program is
committed yet (explicit no-commit instruction this session). Prior: same date, `erp-dashboards`
Phase 3 "Purchase Dashboard" and Phase 4 "Production Dashboard" both ✅ VERIFIED at agent level —
see
`process/features/erp-dashboards/completed/erp-dashboards_18-09-26/phase-03-purchase-dashboard_REPORT_22-09-26.md`
and `phase-04-production-dashboard_REPORT_18-09-26.md`. Phase 3: `/dashboards/purchase` +
`/dashboards/purchase/[poNo]` built over dual-basis totals (invoice 461,140 / PO-committed 727,920,
both fully counted, equal-weight tiles — not one "primary" one "footnoted"), 7-branch PO status
derivation with an unvalidated-caveat badge, non-clamped received/outstanding, supplier breakdown,
`canSeeMoney` server-gated. 5 independent EVL cycles: all Fully-Automated gates green every cycle;
9 Hybrid/Agent-Probe gates could not be independently re-run in ANY cycle — env-blocked by a
recurring "Credential Materialization" session restriction (no `.env` read path), NOT a code
defect — accepted as a known-gap (the EXECUTE session itself ran all 27 Hybrid e2e gates green
with real fixture data). Phase 4: `/dashboards/production` + `/dashboards/production/[moNumber]`
built plan-only — an MO list with NO achievement-% anywhere (enforced structurally + by data-shape
+ module-surface + rendered-page, 4 independent levels), MO status donut, raw-material-issue
drilldown; no money data exists on this dashboard at all (no `canSeeMoney` gate by design, not a
gap). EVL: 1 cycle, `gates_green: true`, zero fix cycles needed — the cleanest EVL result in the
program so far. Both phases: zero schema change, `guardedQuery`-only ERP access, db_TCL never
contacted. Combined unit suite 282→**390 tests/30 files**; e2e suites gained
`dashboards-purchase.spec.ts` (27) and `dashboards-production.spec.ts` (17). Program advances to
Phase 5 (hardening, CSV export, cross-dashboard money-gate audit, full regression, manual
live-reconcile, AC18 boot-probe, rollout readiness); nothing committed yet — see program details in
Current Features below; prior: same date, `erp-dashboards` Phase 2 "Sales Dashboard" ✅ VERIFIED at
agent level, UPDATE PROCESS closeout — see
`phase-02-sales-dashboard_REPORT_22-09-26.md`; `/dashboards/sales` built over the DO/DOdtl basis
via `resolveSalesBasis()`, count/qty-per-unit KPIs, priced-only THB + coverage % + reconciliation
footnote (independently re-confirmed against real `erp_fixture` data: 10,111.00 reconciliation
match, 19.4% coverage, ฿858,937.21 excluded), product/customer drilldown, hand-rolled CSS/SVG
charts (`recharts` never adopted); EVL ran 2 cycles — Cycle 1 (env-blocked) found and fixed a
cold-cache-500 defect (`sales-unavailable.tsx`, new), Cycle 2 is `all_pass: true`; db_TCL never
contacted; zero schema change; unit 212→282 tests/23 files, `dashboards-sales.spec.ts` now 19
tests; nothing committed yet; prior: same date,
`erp-dashboards` Phase 1 "ERP Read Foundation" ✅ VERIFIED at agent level, UPDATE PROCESS closeout
— see `phase-01-erp-read-foundation_REPORT_22-09-26.md`; the read-only ERP pipe (`src/lib/erp/*`),
local `erp_fixture` sandbox DB, `/api/health/erp`, shared `dashboard-data-table.tsx`/
`pilot-banner.tsx`/`degrade-banner.tsx`, and the 3-link "แดชบอร์ด" nav group are all built and
EVL-confirmed green; unit 100→212 tests/20 files, e2e 49→56 excl. `[setup]`); prior: 2026-07-19
(`per-location-shop-numbering` plan ✅ VERIFIED and
...
group) and `sortShopsForDisplay(...)` (unfiltered `/shops` ordering: location asc, null/empty group
LAST, `rosterOrder` asc within group); VALIDATE intentionally skipped (UI-display-only, no
schema/auth/API surface, same shape as `shop-location-filter`); unit suite now 100 tests/16 files
(+8, all in `roster.test.ts`), e2e now 49 tests excl. `[setup]` (50 incl., +1 new `/shops` gate);
prior: `shop-location-filter` plan ✅ VERIFIED AT CODE LEVEL and archived —
per-location LIST filtering added to `/shops` (new "สถานที่" column + a new
`shop-location-filter.tsx` client-component `?location=` dropdown, the codebase's first
`useRouter`-driven filter) and `/orders` (the EXISTING `new-sheet-form.tsx` สถานที่ select now does
double duty — controlled + URL-synced, filters the sheet list AND still supplies the create
default); ZERO schema change (reuses `Shop.location`/`OrderSheet.location` + the existing
`getEffectiveLocationOptions()` helper); VALIDATE intentionally skipped (UI-only, no
schema/auth/API surface); unit suite unchanged at 92 tests/16 files, e2e now 48 tests excl.
`[setup]` (49 incl.); pending-manual: user live-UX sign-off, deploy not yet scheduled; prior:
`location-management` plan ✅ VERIFIED AT CODE LEVEL, committed —
a managed `/locations` list (create/rename/delete) stored as ONE JSON `AppSetting` row
(`key: "locations"`, ZERO schema change); shop-form location field and the order-sheet location
picker now both source from `getEffectiveLocationOptions()` (managed list ∪ distinct existing
`Shop.location` values) instead of free text; rename cascades atomically to every matching
`Shop.location`; delete is blocked while any shop (active or soft-deleted) still references the
location; unit suite now 92 tests/16 files, e2e now 46 tests excl. `[setup]` (47 incl.);
pending-manual: user confirmation of live `/locations` UX on a real browser; prior:
`shop-location-roster` plan ✅ VERIFIED AT CODE LEVEL and archived, commit
`e2d3adb` — `Shop.location` field + per-location shop roster (`src/lib/roster.ts`
`buildLocationRoster`) replacing the fixed 29-slot roster with a variable, `displayNo`-numbered
roster on order entry AND both print routes; new db_TCL delivery script
`db/alter-shop-add-location.sql` (NOT yet run against db_TCL — pending customer deploy step);
prior still: `matrix-print-darkmode-fixes`
plan ✅ VERIFIED at code level and archived — dark-mode `--surface-focus`/`--seasoning-band`
tokens, print footer one-page-fit (e2e gate G9), รอยืนยัน badge removed from /shops+/products;
`remove-settings-db` plan VERIFIED and archived: the `/settings/db` runtime DB-connection-switcher
page removed entirely — DB config is now a manual `.env`-edit + restart ops procedure,
`resolve-database-url.ts` untouched; `responsive-drawer-sidebar` plan CODE DONE and archived:
3-tier responsive app shell — phone bottom-tab-bar unchanged, tablet 768-1024px hidden drawer +
hamburger, desktop ≥1024px fixed sidebar with a functional collapse/reopen — Agent-Probe rows
pending-manual; `ordersheet-soft-delete` follow-up plan VERIFIED and archived: `OrderSheet.active`
soft-delete column + ADMIN-only delete UI; production DB reality captured: `db_TCL` verified as the
customer's live shared ERP database, not a dedicated orderstock DB — see database/all-database.md
DANGER guardrails; db-url-dollar-roundtrip fix landed — `resolveDatabaseUrl()` raw-read resolver
added; production Docker deploy kit built + hosting switched to subdomain `orderstock.krs.co.th` at
domain root, no basePath; pguard-redesign PROGRAM COMPLETE — all 5 phases VERIFIED, archived to
`completed/`; phase1-order-system remains PROGRAM COMPLETE, all 6 phases VERIFIED, archived to
`completed/`)
...
1. **Daily order sheet entry** following the reference document `Scan2026-07-04_170934.pdf` (repo root):
   ...
   - Rows = customer shops (ร้านค้า), ~29 per sheet (each shop now belongs to a `location`
     (สถานที่); the daily sheet picks one location and the roster is filtered + renumbered to just
     that location's shops — added 13-07-26, `shop-location-roster` plan; see
     `database/all-database.md` §Per-Location Shop Roster)
...
| ERP dashboard / read-only ERP query work | `all-context.md`, `database/all-database.md` | the archived program in `process/features/erp-dashboards/completed/erp-dashboards_18-09-26/` (umbrella + all 6 phase plans/reports), plus `erp-dashboards-proposal_REF_18-09-26.md` and `erp-data-dictionary_REF_18-09-26.md` for the tested query patterns; check `process/features/erp-dashboards/backlog/` first for any new dashboard request — new ERP-dashboard work gets its own new task folder |
...
| `erp-dashboards` | `process/features/erp-dashboards/` | **PROGRAM COMPLETE (18-09-26 → 22-09-26)** — 6-phase program (archived to `completed/erp-dashboards_18-09-26/`) adding 3 read-only dashboards (Sales/Purchase/Production) over the customer's shared ERP database `db_TCL`, via a separate guarded `mssql` connection pool (never Prisma). All 6 phases ✅ VERIFIED at agent level: **Phase 0** prerequisites (DBA login delivery script, question list). **Phase 1** ERP read foundation — `src/lib/erp/*` guarded read layer (5-layer read-only defense: normalizer, 19-rule denylist, parameterized-only `guardedQuery` choke point, `HAS_PERMS_BY_NAME` boot probe, `ApplicationIntent=ReadOnly`, compile-time no-write-method type guard), separate `mssql` pool, 5-min TTL cache with degrade, `/api/health/erp`, shared `dashboard-data-table.tsx`/`pilot-banner.tsx`/`degrade-banner.tsx`, "แดชบอร์ด" nav group, LOCAL-sandbox-only `erp_fixture` DB. **Phase 2** `/dashboards/sales` — DO basis, `resolveSalesBasis()`, count/qty-per-unit KPIs, priced-only THB + coverage % + reconciliation footnote, product/customer drilldown, hand-rolled CSS/SVG charts. **Phase 3** `/dashboards/purchase` — dual-basis totals (invoice 461,140 / PO-committed 727,920), 7-branch PO status derivation, non-clamped outstanding, supplier breakdown. **Phase 4** `/dashboards/production` — plan-only MO list with NO achievement-% anywhere (4-level enforcement), MO status donut, material-issue drilldown, no money data at all (by design). **Phase 5** hardening/export/rollout — CSV export on all six dashboard tables (UTF-8 BOM, Thai headers, BE dates, STAFF money columns omitted entirely), cross-dashboard AC9 money audit (98 tests), first real ERP force-down mechanism (`force-down.ts` + `/api/test/erp-force-down`), permanent automated SQL-guard sweep over all 13 query files, fixture-seed order-independence fix, USER-RUN live-reconcile script (never run against `db_TCL`), rollout-readiness REF, deployment guide §12. **Final counts:** unit 523 tests/32 files with ERP fixture wired (490/33 self-skipped without); e2e 145 passed/7 skipped (mobile+tablet+desktop+setup projects). Zero schema change across the whole program; no ERP table ever entered `prisma/schema.prisma`; `db_TCL` never contacted by any automated run — `guardedQuery`-only access confirmed by static audit every phase. **Durable tooling lesson:** every `vc-tester` EVL subagent sandbox is blocked from materializing `ERP_DATABASE_URL`/`ERP_ALLOW_WRITE_CAPABLE_LOGIN` — ERP-env-dependent gates must be confirmed by the orchestrator or user directly (see `tests/all-tests.md`), not by a subagent; this recurred identically across Phases 2/3/5 and closed a false "known-gap" on Phase 3 once corrected. **Production go-live gated on 2 permanent USER-RUN items** (DBA-provisioned scoped read-only login; the manual live reconcile itself) — tracked in `process/features/erp-dashboards/backlog/`. Nothing from this program is committed yet. Any new ERP-dashboard work (new dashboard, new export target, phone route, the app-wide button-contrast accessibility pass) is a NEW task in a fresh task folder — this program folder should not be reopened. |
...
orderstock/
  ...
  prisma/
    schema.prisma             -- full 9-model domain schema (Phase 02) — see database/all-database.md; `shop-location-roster_13-07-26` added `Shop.location String? @db.NVarChar(200)`
    migrations/                -- migrations incl. init_healthcheck, phase02_full_schema, shop_rosterorder_unique, 20260713000000_shop_location (hand-authored ALTER — see database/all-database.md)
  ...
  db/
    ...
    alter-shop-add-location.sql -- shop-location-roster_13-07-26: idempotent `IF COL_LENGTH(...) IS NULL ... ADD` delivery ALTER for Shop.location; NOT yet run against db_TCL (pending customer deploy step)
  docs/
  ...
  e2e/                         -- Playwright specs (Phase 03) — see auth/all-auth.md E2E fixtures section
    ...
    orders.spec.ts              -- Phase 04: D1/D2 order-sheet round-trip + snapshot-preserve hybrid gates; shop-location-roster_13-07-26 added G4 (per-location filter/displayNo) + G5 (null-location fallback); test isolation switched from free-text location tags to a unique DATE (`dropByDate`)
    print.spec.ts               -- Phase 05: print gates (colgroup 24/20, 29 rows + 446 totals, @page A4 landscape, snapshot-render + restore-in-finally, per-shop .sheet/break, test-side page.pdf, unauth redirect, one-page-fit G9); shop-location-roster_13-07-26 added a per-location variable-row print gate (G6 in that plan's own table)
    ...
    shops.spec.ts                -- shop-location-filter_17-07-26 (✅ VERIFIED AT CODE LEVEL, NEW file): สถานที่ column renders + the new filter dropdown narrows the /shops list via ?location=, ทุกสถานที่ restores all
    util/
  ...
  src/
    ...
    app/
      ...
      (main)/                   -- Phase 05: route GROUP (adds no URL segment) holding every authenticated app route; layout.tsx composes sidebar (nav.tsx) + topbar.tsx + scrollable <main> (pguard-redesign Phase 01)
        ...
        shops/**                -- shops master-data CRUD (Phase 02; requireAuth-guarded since Phase 03); shop-location-roster_13-07-26 added an optional "สถานที่" (`location`) input to shop-form.tsx + a `.max(200)` zod field in actions.ts; shop-location-filter_17-07-26 (✅ VERIFIED AT CODE LEVEL) added a "สถานที่" column to page.tsx + a NEW shop-location-filter.tsx client component (`?location=` searchParam filter, the codebase's first useRouter-driven filter); per-location-shop-numbering_19-07-26 (✅ VERIFIED) changed page.tsx's "ลำดับ" column to render a per-location display number (via roster.ts's new perLocationDisplayNo map) instead of the global rosterOrder, and unfiltered fetches are now sorted via sortShopsForDisplay (location groups, null-location last)
        ...
        orders/**               -- daily order-sheet entry: list/create/edit by date+location (Phase 04); [id]/page.tsx extended Phase 05 with พิมพ์รวมทั้งวัน/พิมพ์แยกร้าน print links; pguard-redesign Phase 02 REPLACED order-grid.tsx (+ shop-rail/shop-order-card/summary-bar, all 4 deleted) with order-matrix.tsx; `ordersheet-soft-delete` plan (11-07-26, VERIFIED) added ADMIN-only soft-delete (`OrderSheet.active`); `shop-location-roster_13-07-26` (✅ VERIFIED AT CODE LEVEL) replaced the fixed `ROSTER_SLOTS=29` loop in `[id]/page.tsx` with `src/lib/roster.ts`'s `buildLocationRoster()`; `location-management_14-07-26` (✅ VERIFIED AT CODE LEVEL) replaced `page.tsx`'s inline `distinct` shop-location query with `src/lib/locations.ts`'s `getEffectiveLocationOptions()` (managed list ∪ legacy shop-only values); `shop-location-filter_17-07-26` (✅ VERIFIED AT CODE LEVEL) added a `?location=` searchParam filter to page.tsx's `orderSheet.findMany` and made new-sheet-form.tsx's สถานที่ select do double duty (controlled + URL-synced filter, still supplies the create default) — see database/all-database.md
        locations/**            -- location-management_14-07-26 (✅ VERIFIED AT CODE LEVEL, NEW): managed สถานที่ list page — page.tsx (server, requireAuth no role, force-dynamic), actions.ts (createLocationAction/renameLocationAction/deleteLocationAction), locations-manager.tsx (client, per-row LocationRow subcomponent each with its own useActionState — mirrors delete-sheet-button.tsx's confirm-modal pattern); rename cascades atomically to Shop.location via prisma.$transaction; delete blocked while any shop (active OR soft-deleted) still references the location
          order-matrix.tsx       -- pguard-redesign Phase 02: 20-col daily-order MATRIX (3-tier data-driven header, KPI strip, totals row via totals.ts) — desktop order-entry surface; hidden-input payload built EXCLUSIVELY via src/lib/order-payload.ts (byte-identical to the retired Order Pad); save/print portaled into #topbar-actions (see uxui/all-uxui.md); Phase 04: adds a md:hidden mobile branch (list+entry) over the SAME cells/notes state + form + buildOrderPayload — desktop matrix wrapped hidden md:flex; shop-location-roster_13-07-26: `GridRow` gains `displayNo` (visible row number), `rosterOrder` still keys `data-testid`/React `key`
          order-mobile-list.tsx  -- pguard-redesign Phase 04: NEW presentational mobile shop-list sub-state (imported by order-matrix.tsx, same setCell/setNote setters); shop-location-roster_13-07-26: consumes the same `displayNo`/`rosterOrder` split automatically (no own logic change)
          ...
          new-sheet-form.tsx     -- native date input (CE) + read-only BE label + "วันนี้" shortcut; shop-location-roster_13-07-26: location field is now a `<select>` of distinct, `active:true`-filtered shop locations (was free text); shop-location-filter_17-07-26: location `<select>` is now CONTROLLED (`value={selectedLocation}`) with an onChange that navigates `/orders?location=...`, doing double duty as both the list filter and the create-default — `name="location"` and createOrderSheet's contract are unchanged
      ...
      print/                    -- Phase 05: dedicated NO-NAV route group (chrome-free /print), sibling of (main)/, NOT nested inside it
        ...
        daily/[date]/page.tsx   -- combined-day sheet: variable per-location roster (was fixed 29 roster slots incl. blank gaps, pre-`shop-location-roster_13-07-26`)
        shops/[date]/page.tsx   -- per-shop sheets, one `.sheet{break-after:page}` per shop, `?slots=` filter (unchanged, keys on global `rosterOrder`)
        print-table.tsx         -- pure server render: 24-physical/20-semantic-col table, mm <colgroup>, totals row = LAST tbody row (never tfoot); shop-location-roster_13-07-26: row-number `<td>` uses the new `displayNo`, React `key` stays `rosterOrder`
    ...
    lib/
      ...
      get-sheet-for-print.ts    -- Phase 05: NEW snapshot-only fetch (shopNameAtEntry/variantNameAtEntry, NEVER live Shop/ProductVariant names) — shared by both print routes; daily/per-shop both now render a variable per-location roster via `roster.ts`'s `buildLocationRoster()` (was a fixed 29-slot loop pre-`shop-location-roster_13-07-26`)
      roster.ts                 -- shop-location-roster_13-07-26 (NEW): pure `buildLocationRoster(activeShops, sheetLocation)` — the single source of truth for the per-location shop filter + null/no-match fallback + `rosterOrder`-asc sort + NEW `displayNo` (1..N) renumbering; imported by BOTH `orders/[id]/page.tsx` and `get-sheet-for-print.ts`; per-location-shop-numbering_19-07-26 (✅ VERIFIED) added two more pure, DB-free exports — `perLocationDisplayNo(activeShops): Map<shopId, number>` (per-location 1..N numbering across ALL locations at once, no fallback) and `sortShopsForDisplay(shops)` (location-group ordering, null-location group last, rosterOrder asc within group) — imported by `shops/page.tsx` only; `buildLocationRoster` itself is untouched — see database/all-database.md
      ...
      __tests__/                -- smoke, variant-validation, correction-cascade, password, login-attempts, auth-guard-coverage (now covers (main)/ + print pages + settings), secret-leak, totals, be-date, order-save, roster, locations (pure-transform unit tests for locations-core.ts) (100 tests total/16 files, after remove-settings-db plan removed connection-string.test.ts + env-write.test.ts + settings-secret-hygiene.test.ts, then shop-location-roster_13-07-26 added roster.test.ts +5, then location-management_14-07-26 added locations.test.ts +17, then per-location-shop-numbering_19-07-26 added +8 more tests to roster.test.ts (perLocationDisplayNo + sortShopsForDisplay describe blocks) netting 92→100, still 16 files)
  test-fixtures/
    sheet-13-03-69.json         -- canonical 13/3/69 gate fixture (51 cells, 20 column totals, grand 446, 13 NoteLines) — shared by Phase 04 unit gate + Phase 05 print e2e tests + roster.test.ts (shop-location-roster_13-07-26); `location` field set to `"ยิ่งเจริญ"`
...
- **Testing:** Vitest 3.2.6 (`pnpm test` → `vitest run`) — 100 tests across 16 files (grew to 99/17 through the `ordersheet-soft-delete_11-07-26` plan; the `remove-settings-db_11-07-26` plan then DELETED `connection-string.test.ts`/`env-write.test.ts`/`settings-secret-hygiene.test.ts` and pruned 3 `auth-guard-coverage.test.ts` assertions, netting 99/17 → 70/14; `shop-location-roster_13-07-26` then ADDED `roster.test.ts` (+5), netting 70/14 → 75/15; `location-management_14-07-26` then ADDED `locations.test.ts` (+17), netting 75/15 → 92/16; `shop-location-filter_17-07-26` added no new unit tests — netting still 92/16; `per-location-shop-numbering_19-07-26` then ADDED +8 more tests to the existing `roster.test.ts` (new `perLocationDisplayNo`/`sortShopsForDisplay` describe blocks), netting 92/16 → **100/16**). Playwright `@playwright/test@1.61.1` — 49 E2E tests (excl. `[setup]`, 50 incl.) across `e2e/auth.spec.ts` + `e2e/orders.spec.ts` + `e2e/print.spec.ts` + `e2e/summary-history.spec.ts` + `e2e/mobile.spec.ts` (390×844 `mobile` project) + `e2e/shops.spec.ts` + `e2e/sidebar-drawer.spec.ts` (820×1180 `tablet` project) + `e2e/sidebar-desktop-collapse.spec.ts` (`chromium` project) + `e2e/auth.setup.ts` fixtures — `remove-settings-db_11-07-26` DELETED `e2e/settings.spec.ts` (34→30), `responsive-drawer-sidebar_11-07-26` added the new `tablet` project spec (5 gates) + the new desktop-collapse spec (3 gates) (30→38), `matrix-print-darkmode-fixes_13-07-26` added print gate G9 (38→39), `shop-location-roster_13-07-26` added G4/G5 (`orders.spec.ts`) + G6 (`print.spec.ts`) (39→42), `location-management_14-07-26` added 4 new location-management gates to `e2e/orders.spec.ts` (42→46), `shop-location-filter_17-07-26` added 1 new gate to `e2e/orders.spec.ts` + 1 new gate in `e2e/shops.spec.ts` (46→48), then `per-location-shop-numbering_19-07-26` added 1 new gate to the existing `e2e/shops.spec.ts` (48→**49**). See `tests/all-tests.md`. **pguard-redesign PROGRAM COMPLETE (08-07-26); order-system `ordersheet-soft-delete`, `remove-settings-db`, `shop-location-roster`, `location-management`, `shop-location-filter`, and `per-location-shop-numbering` follow-up plans VERIFIED/verified-at-code-level (latest 19-07-26); `responsive-drawer-sidebar` plan CODE DONE (11-07-26, Agent-Probe rows pending-manual).**
...
- **Per-location shop roster (`shop-location-roster_13-07-26`, ✅ VERIFIED AT CODE LEVEL):** `Shop` gained a nullable `location` field; the fixed `ROSTER_SLOTS = 29` roster loop (formerly hardcoded twice — `orders/[id]/page.tsx` and `get-sheet-for-print.ts`) is replaced by ONE shared pure helper, `src/lib/roster.ts` `buildLocationRoster(activeShops, sheetLocation)`, that filters shops to the sheet's location, falls back to the full active-shop list on a null/no-match location, and assigns a NEW per-location `displayNo` (1..N) used ONLY for the visible row number. The global, `@unique` `rosterOrder` field is UNCHANGED and still drives `data-testid`/React `key`/print `?slots=` filtering — the two fields are deliberately never conflated (see `database/all-database.md` §Per-Location Shop Roster for the full contract and the db_TCL delivery-script deploy ordering). Any future roster-adjacent screen must import this one helper rather than re-deriving the filter/fallback/renumber logic.
- **Managed location list (`location-management_14-07-26`, ✅ VERIFIED AT CODE LEVEL):** the free-text `location` field used to be typed independently on every shop-edit form and the order-sheet picker only showed values that already existed on ≥1 shop, letting near-duplicate strings fragment the roster's exact-match filter. `src/lib/locations.ts` (DB helpers: `getManagedLocations` lazy-seed-from-existing-shops-on-first-read, `setManagedLocations`, `getEffectiveLocationOptions` = managed list ∪ distinct active-shop values) + `src/lib/locations-core.ts` (pure transforms: `normalizeLocations`/`addLocation`/`renameLocation`/`removeLocation`) store the ONE authoritative list as a JSON array under a single `AppSetting` row (`key: "locations"`) — ZERO schema change, same pattern as `app-settings.ts`. A new `/locations` page (both ADMIN+STAFF, same access as `/shops`/`/products`) lets staff create/rename/delete entries; rename cascades ATOMICALLY to every matching `Shop.location` via `prisma.$transaction` with a graceful error path; delete is blocked (Thai error naming the shop count) while any shop — active OR soft-deleted — still references the location. `shop-form.tsx`'s location field became a `<select>` sourced from the managed list (with a defensive fallback option for a shop's current value if it was since deleted from the list); `orders/page.tsx`'s location picker now sources `getEffectiveLocationOptions()` instead of its own inline `distinct` query. `Shop.location` stays `String?`; `roster.ts`'s exact-match filter is completely untouched — this plan only changes WHERE the string value comes from. Accepted known-gap: concurrent read-modify-write race on the single `AppSetting` "locations" row (mirrors the existing `OrderSheet` date+location TOCTOU precedent).
- **Per-location list filtering — /shops + /orders (`shop-location-filter_17-07-26`, ✅ VERIFIED AT CODE LEVEL):** a `?location=` searchParam is the single source of truth for two independent list-filter surfaces, reusing `getEffectiveLocationOptions()` and ZERO schema change. `/shops` gained a "สถานที่" column (`shop.location ?? "-"`) and a NEW `shop-location-filter.tsx` client component (the codebase's first `useRouter`-driven filter — an uncontrolled `<select>` that navigates on change). `/orders`'s EXISTING `new-sheet-form.tsx` สถานที่ select does double duty — it became CONTROLLED (`value={selectedLocation}`, sourced from the URL) and its `onChange` navigates the same way, while `name="location"` stays unchanged so it still supplies `createOrderSheet`'s create-time value (the currently-filtered location). VALIDATE was intentionally skipped (UI-only, no schema/auth/API surface). See `uxui/all-uxui.md` §Per-location list filtering for the full component/pattern detail.
- **Per-location shop DISPLAY numbering — /shops (`per-location-shop-numbering_19-07-26`, ✅ VERIFIED):** the `/shops` "ลำดับ" column now shows a per-location 1..N display number instead of the shop's global `rosterOrder`, matching how the daily order sheet already numbers shops (`displayNo`). Two new pure, DB-free exports on `src/lib/roster.ts` — `perLocationDisplayNo(activeShops): Map<shopId, number>` (groups active shops by `location?.trim() || ""`, sorts by `rosterOrder` asc within each group, assigns 1..N; a shop with a null/empty location lands in its own "no location" bucket, no fallback-to-full-list behavior, unlike `buildLocationRoster`) and `sortShopsForDisplay(shops)` (unfiltered `/shops` ordering: location group asc with the null/empty bucket always LAST, `rosterOrder` asc within group). `shops/page.tsx` fetches all shops once (dropped the conditional DB `where`), computes the map over active shops only, and renders `displayNoMap.get(shop.id) ?? "-"` in the ลำดับ `<td>` (inactive/soft-deleted shops always show `-`). ZERO schema change; `rosterOrder` (the `@unique` DB identity field), `buildLocationRoster`, the order sheet, and both print routes are completely untouched. VALIDATE was intentionally skipped (UI-display-only, no schema/auth/API surface, same shape as `shop-location-filter`). See `uxui/all-uxui.md` §Per-location shop display numbering and `database/all-database.md` §Per-Location Shop Roster for the full field-split rationale (`rosterOrder` = global identity, never renumbered; `displayNo`/`perLocationDisplayNo` = presentation only).
...
- `db/alter-shop-add-location.sql` not yet run against `db_TCL` (`shop-location-roster_13-07-26`, 13-07-26) — pending customer/DBA deploy step; the app's null-location fallback keeps it safe to deploy code before this runs, but the roster stays un-filtered (full active-shop list) until the ALTER + backfill land
- User confirmation of live per-location roster behavior on a real sheet (`shop-location-roster_13-07-26`) — all automated/hybrid gates are green; this plan's own completion rules call for an explicit manual sign-off beyond that before calling it fully verified
- User confirmation of live `/locations` UX on a real browser (`location-management_14-07-26`) — all Fully-Automated/Hybrid gates are green; the plan's own completion rules call for an explicit manual sign-off beyond that before calling it fully verified
- User live-UX sign-off on the `/shops` + `/orders` per-location filter (`shop-location-filter_17-07-26`) — all Fully-Automated/Hybrid gates are green (code-level verified); deployment of this change to production is a separate, not-yet-scheduled step
- Agent-Probe manual visual scan of the unfiltered `/shops` list (`per-location-shop-numbering_19-07-26`) — all Fully-Automated/Hybrid gates are green (full EVL confirmation run); the plan's own Test Plan calls for a person's visual confirmation of grouped-list readability before treating this row as independently re-confirmed

## Scan Metadata

- Generated: 2026-09-22 (`erp-dashboards` **PROGRAM COMPLETE** — Phase 5 "Hardening, Export & Rollout" ✅ VERIFIED at agent level; program archived to `completed/erp-dashboards_18-09-26/` — UPDATE PROCESS closeout)
- HEAD: d4e084e (this session's changes are uncommitted in the working tree by explicit no-commit instruction — see git status; a separate vc-git-manager pass handles the execution commit followed by a process commit, per the umbrella's commit-hygiene rule)
- Mode: delta update (erp-dashboards Phase 5: CSV export on all six dashboard tables, cross-dashboard money audit, ERP force-down mechanism, permanent SQL-guard sweep, fixture-seed order-independence fix, live-reconcile script, rollout readiness; corrected Phase 3's "9 gates env-blocked" known-gap — independently closed by the orchestrator's own full-suite run; test-count refresh 390→523 unit/30→32 files with ERP wired (490/33 without), e2e 119→145/7 skipped)
- Package manager: pnpm 11.5.0
- Prior: 2026-09-22 (`erp-dashboards` Phase 3 "Purchase Dashboard" + Phase 4 "Production Dashboard" both ✅ VERIFIED at agent level — combined UPDATE PROCESS closeout), HEAD 593dee5
