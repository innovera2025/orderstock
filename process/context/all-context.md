...
Last updated: 2026-07-13 (`shop-location-roster` plan ✅ VERIFIED AT CODE LEVEL and archived, commit
...
`db/alter-shop-add-location.sql` (NOT yet run against db_TCL — pending customer deploy step); unit
suite now 75 tests/15 files, e2e now 42 tests excl. `[setup]` (43 incl.); pending-manual: user
confirmation of live per-location behavior on a real sheet; prior: `matrix-print-darkmode-fixes`
...
| `tests/` | `process/context/tests/all-tests.md` | Testing entrypoint for orderstock — Vitest 3.2.6 (75 tests/15 files) and Playwright E2E (42 tests excl. [setup], incl. mobile + tablet projects) both real and wired, sandbox SQL Server constraint |
...
orderstock/
  ...
  src/
    ...
    lib/
      ...
      __tests__/                -- smoke, variant-validation, correction-cascade, password, login-attempts, auth-guard-coverage (now covers (main)/ + print pages + settings), secret-leak, totals, be-date, order-save, roster (NEW, shop-location-roster_13-07-26) (75 tests total/15 files, after remove-settings-db plan removed connection-string.test.ts + env-write.test.ts + settings-secret-hygiene.test.ts, then shop-location-roster_13-07-26 added roster.test.ts +5)
  test-fixtures/
...
- **Testing:** Vitest 3.2.6 (`pnpm test` → `vitest run`) — 75 tests across 15 files (grew to 99/17 through the `ordersheet-soft-delete_11-07-26` plan; the `remove-settings-db_11-07-26` plan then DELETED `connection-string.test.ts`/`env-write.test.ts`/`settings-secret-hygiene.test.ts` and pruned 3 `auth-guard-coverage.test.ts` assertions, netting 99/17 → 70/14; `shop-location-roster_13-07-26` then ADDED `roster.test.ts` (+5), netting 70/14 → **75/15**). Playwright `@playwright/test@1.61.1` — 42 E2E tests (excl. `[setup]`, 43 incl.) across `e2e/auth.spec.ts` + `e2e/orders.spec.ts` + `e2e/print.spec.ts` + `e2e/summary-history.spec.ts` + `e2e/mobile.spec.ts` (390×844 `mobile` project) + `e2e/sidebar-drawer.spec.ts` (820×1180 `tablet` project) + `e2e/sidebar-desktop-collapse.spec.ts` (`chromium` project) + `e2e/auth.setup.ts` fixtures — `remove-settings-db_11-07-26` DELETED `e2e/settings.spec.ts` (34→30), `responsive-drawer-sidebar_11-07-26` added the new `tablet` project spec (5 gates) + the new desktop-collapse spec (3 gates) (30→38), `matrix-print-darkmode-fixes_13-07-26` added print gate G9 (38→39), then `shop-location-roster_13-07-26` added G4/G5 (`orders.spec.ts`) + G6 (`print.spec.ts`) (39→**42**). See `tests/all-tests.md`. **pguard-redesign PROGRAM COMPLETE (08-07-26); order-system `ordersheet-soft-delete`, `remove-settings-db`, and `shop-location-roster` follow-up plans VERIFIED/verified-at-code-level (latest 13-07-26); `responsive-drawer-sidebar` plan CODE DONE (11-07-26, Agent-Probe rows pending-manual).**
...
- Generated: 2026-07-13 (`shop-location-roster` plan ✅ VERIFIED AT CODE LEVEL, archived — UPDATE-PROCESS)
- HEAD: e2d3adb (feat(orders): per-location shop roster — pick a location, see only its shops)
- Mode: delta update (Shop.location + per-location roster helper capture; test-count refresh 70→75 unit, 39→42 e2e excl. setup)
