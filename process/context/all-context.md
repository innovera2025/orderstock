...
Last updated: 2026-09-22 (`erp-dashboards` Phase 3 "Purchase Dashboard" and Phase 4 "Production
Dashboard" both ✅ VERIFIED at agent level, combined UPDATE PROCESS closeout — see
`process/features/erp-dashboards/active/erp-dashboards_18-09-26/phase-03-purchase-dashboard_REPORT_22-09-26.md`
...
| `tests/` | `process/context/tests/all-tests.md` | Testing entrypoint for orderstock — Vitest 3.2.6 (390 tests/30 files, 24 skipped self-guarding Hybrid gates + 1 todo) and Playwright E2E (49+ tests excl. [setup], incl. mobile + tablet projects, plus the erp-dashboards Sales/Purchase/Production suites) both real and wired, sandbox SQL Server constraint |
...
| `erp-dashboards` | `process/features/erp-dashboards/` | **IN PROGRESS (started 18-09-26)** — 6-phase program (`erp-dashboards_18-09-26`) adding 3 read-only ERP dashboards (Sales/Purchase/Production) over the customer's shared ERP database `db_TCL`, via a separate guarded `mssql` connection pool (never Prisma). Phase 0 (Prerequisites) done; **Phase 1 (ERP read foundation) ✅ VERIFIED at agent level, 22-09-26** (EVL confirmation run independently re-ran all gates; `gates_green: true`, no fix cycle) — `src/lib/erp/*` guarded read layer (5-layer read-only defense: normalizer, 19-rule denylist, parameterized-only `guardedQuery` choke point, `HAS_PERMS_BY_NAME` boot probe, `ApplicationIntent=ReadOnly`, plus a compile-time no-write-method type guard), lazy separate `mssql` pool + `ERP_DATABASE_URL` raw-read resolver, 5-min TTL cache with last-known-good degrade, public `/api/health/erp`, shared `dashboard-data-table.tsx`/`pilot-banner.tsx`/`degrade-banner.tsx`, the 3-link "แดชบอร์ด" nav group, and a LOCAL-sandbox-only `erp_fixture` database (`db/erp-fixture/*.sql`). Unit suite 100 → 212 tests/20 files; e2e 49 → 56. Zero schema change; no ERP table in `prisma/schema.prisma`; db_TCL never contacted. Known gap: the live boot probe against the real DBA-provisioned scoped read-only login (Phase 5, deferred, non-blocking). Nothing committed yet. **Phase 2 (Sales dashboard) ✅ VERIFIED at agent level, 22-09-26** (EVL confirmation ran 2 cycles; Cycle 1 was env-blocked but found and fixed a real cold-cache-500 defect; Cycle 2, with the local `erp_fixture` env configured, is `all_pass: true`/`gates_green: true`) — `/dashboards/sales` is a server-component dashboard over the DO basis (`tbl_DOhdr`/`tbl_Dodtl`) read exclusively through Phase 1's `guardedQuery()`: versioned SQL in `db/erp-queries/sales/*.sql` (embedded byte-identically via `src/lib/sales-sql.ts`, drift-gated by a unit test, because `output: "standalone"` does not ship `db/`), pure helpers in `src/lib/sales-basis-core.ts`, the `AppSetting`-backed `resolveSalesBasis()` switch mechanism (DO branch only), a URL-searchParam state machine (summary → DO list → DO lines), a สัปดาห์/เดือน/ปี period toggle feeding BOTH bar charts, a status donut + category pie with cross-filter-but-self-unfiltered semantics, and ADMIN-only money gated SERVER-SIDE (Staff HTML contains no money markup at all, independently re-confirmed against real `erp_fixture` data: reconciliation match 10,111.00=10,111.00, 19.4% coverage, ฿858,937.21 excluded). A new `sales-unavailable.tsx` (EVL fix) now shows a Thai unavailable state instead of a 500 on a cold-cache ERP outage. Quantities are aggregated strictly per unit. Charts are HAND-ROLLED CSS/SVG — `recharts` was never installed (see the phase report's Charting Decision). ZERO schema change; no ERP model in `prisma/schema.prisma`; db_TCL never contacted. Unit suite 212 → 282 tests / 23 files; e2e `dashboards-sales.spec.ts` is now 19 tests (18 pass + 1 self-skip when ERP is healthy). **Phase 3 (Purchase dashboard) ✅ VERIFIED at agent level, 22-09-26** (5 independent EVL cycles: all Fully-Automated/SQL-drift gates green every cycle; the 9 Hybrid/Agent-Probe gates env-blocked in every cycle by a recurring tester-session credential-access restriction — not a code defect, accepted as known-gap, since the EXECUTE session itself ran all 27 Hybrid e2e gates green against real `erp_fixture` data) — `/dashboards/purchase` + `/dashboards/purchase/[poNo]`: dual-basis totals (invoice 461,140 / PO-committed 727,920, both fully counted, equal-weight tiles), 7-branch `derivePoStatus()` with an unvalidated-caveat badge, non-clamped `computeOutstanding()`, supplier breakdown, `canSeeMoney` server-gated identically to Sales. Hand-rolled CSS bars reusing Phase 2's `sales-chart-scale.ts` read-only. **Phase 4 (Production dashboard) ✅ VERIFIED at agent level, 22-09-26** (1 independent EVL cycle, `gates_green: true`, zero fix cycles needed — the cleanest EVL result in the program) — `/dashboards/production` + `/dashboards/production/[moNumber]`: plan-only MO list with NO achievement-% anywhere (enforced at 4 independent levels — structural/data-shape/module-surface/rendered-page), MO status donut with a small-sample caveat, raw-material-issue drilldown; no money data exists on this dashboard at all (no `canSeeMoney` gate by design). Both phases: zero schema change; no ERP table in `prisma/schema.prisma`; db_TCL never contacted; `guardedQuery`-only ERP access confirmed by static audit. Combined unit suite 282 → **390 tests / 30 files**; e2e gained `dashboards-purchase.spec.ts` (27 tests) and `dashboards-production.spec.ts` (17 tests). One additive, self-resolved shared-fixture-table collision (`InventoryFlowHdr`/`InventoryFlowDtl`, used by both Purchase and Production) was found and fixed within Phase 3's own seed file — see `database/all-database.md`. Nothing committed yet from Phases 2, 3, or 4. Program advances to Phase 5 (Hardening, Export & Rollout — cross-dashboard money-gate audit, CSV export, full regression, manual live-db_TCL reconcile, AC18 boot-probe, production rollout readiness). See the umbrella plan for the full charter and hard safety constraints (read-only ERP access, no `sa`/`orderstock_app` login on the ERP pool, money hidden server-side for STAFF). |
...
- Generated: 2026-09-22 (`erp-dashboards` Phase 3 "Purchase Dashboard" + Phase 4 "Production Dashboard", both ✅ VERIFIED at agent level — combined UPDATE PROCESS closeout)
- HEAD: 593dee5 (Phase 3/4 changes are uncommitted in the working tree — see git status; a separate vc-git-manager pass handles the commit(s), execution and process changes split per the umbrella's commit-hygiene rule; Phase 1's own execution commit is also still outstanding)
- Mode: delta update (erp-dashboards Phase 3: Purchase dashboard, dual-basis totals, PO status derivation, non-clamped outstanding, supplier breakdown; Phase 4: Production dashboard, plan-only MO list with 4-level no-achievement-% enforcement, MO status donut, material-issue drilldown; both phases' `auth-guard-coverage.test.ts` entries appended this session; test-count refresh 282→390 unit/23→30 files, e2e gained `dashboards-purchase.spec.ts` (27) + `dashboards-production.spec.ts` (17))
- Package manager: pnpm 11.5.0
- Prior: 2026-09-22 (`erp-dashboards` Phase 2 "Sales Dashboard" ✅ VERIFIED at agent level — UPDATE PROCESS), HEAD bd11700
