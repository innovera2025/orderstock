...
| `tests/` | `process/context/tests/all-tests.md` | Testing entrypoint for orderstock — Vitest 3.2.6 (99 tests/17 files) and Playwright E2E (34 tests, incl. mobile project) both real and wired, sandbox SQL Server constraint |
...
|---|---|---|
| `order-system` | `process/features/order-system/` | **COMPLETE (07-07-26); +1 follow-up plan VERIFIED (11-07-26)** — phase program `phase1-order-system_06-07-26` (umbrella + 6 phase plans), all 6 phases ✅ VERIFIED, archived to `completed/`. Follow-up plan `ordersheet-soft-delete_11-07-26` (ADMIN-only OrderSheet soft-delete, additive `active` column) also ✅ VERIFIED and archived to `completed/`. 7 backlog items remain, pending customer answers or optional hardening — see `process/features/order-system/backlog/` |
...
- **Testing:** Vitest 3.2.6 (`pnpm test` → `vitest run`) — 99 tests across 17 files (pguard-redesign Phase 05 baseline was 88/16; growth to 98/16 from post-program deploy/db work (db-url-dollar-roundtrip fix, db_TCL guardrails); +1 to 99/17 from the `ordersheet-soft-delete_11-07-26` plan's P4 per-function ADMIN-gate regression assertion in `auth-guard-coverage.test.ts`). Playwright `@playwright/test@1.61.1` — 34 E2E tests across `e2e/auth.spec.ts` + `e2e/orders.spec.ts` + `e2e/print.spec.ts` + `e2e/settings.spec.ts` + `e2e/summary-history.spec.ts` + `e2e/mobile.spec.ts` (390×844 `mobile` project) + `e2e/auth.setup.ts` fixtures — the `ordersheet-soft-delete_11-07-26` plan added 9 new gates across `orders.spec.ts`/`auth.spec.ts`/`summary-history.spec.ts`/`print.spec.ts` (25→34) for soft-delete coverage. See `tests/all-tests.md`. **pguard-redesign PROGRAM COMPLETE (08-07-26); order-system `ordersheet-soft-delete` follow-up plan VERIFIED (11-07-26).**
...
- Generated: 2026-07-11 (`ordersheet-soft-delete` plan VERIFIED and archived — UPDATE-PROCESS)
- HEAD: c46d882 (feat(orders): ADMIN-only soft-delete for order sheets)
- Mode: delta update (OrderSheet.active soft-delete + delete-sheet-button.tsx capture)
