...
/** Verbatim copy of `db/erp-queries/production/mo-date-range.sql`. */
export const MO_DATE_RANGE_SQL = `-- ช่วงข้อมูล — Production: what the ERP ACTUALLY holds, regardless of the page's date filter.
--
...
export const PRODUCTION_SQL_SOURCES: ReadonlyArray<{ name: string; file: string; sql: string }> = [
  ...
  { name: "MO_DATE_RANGE_SQL", file: "db/erp-queries/production/mo-date-range.sql", sql: MO_DATE_RANGE_SQL },
];
